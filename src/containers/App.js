		...
		;(async () => {
			...
			for (let city of cleanCityData) {
				const weatherResponse = await fetch(
					`${REACT_APP_CORS_ANYWHERE}/http://api.openweathermap.org/data/2.5/weather?lat=${city.latitude}&lon=${city.longitude}&appid=${REACT_APP_WEATHER_API_KEY}&units=metric`
				)
				const weatherData = await weatherResponse.json()
				const timezoneResponse = await fetch(
					`${REACT_APP_CORS_ANYWHERE}/http://api.geonames.org/timezoneJSON?lat=${city.latitude}&lng=${city.longitude}&username=abdev`
				)
